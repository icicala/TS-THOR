...
class MapperJsonV1(MapperJsonBase):
    ...
    def _get_thor_timestamp(self, json_log: Dict[str, Any]) -> DatetimeField:
        ...
        if not isinstance(thor_timestamp, str):
            raise TimestampError(f"Missing required '{self.__class__.THOR_TIMESTAMP_FIELD}' field in JSON log")
        return DatetimeField(path=self.__class__.THOR_TIMESTAMP_FIELD, datetime=thor_timestamp)
